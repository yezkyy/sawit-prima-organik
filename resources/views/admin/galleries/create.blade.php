...
@section('content')
    ...
    <div class="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            ...
            <h1 class="font-heading text-2xl font-bold text-gray-900">Unggah Foto Galeri</h1>
            <p class="text-gray-500 text-sm mt-1">Tambahkan foto baru ke album dokumentasi website.</p>
        </div>
    </div>
    ...
    <div class="bg-white rounded-[2rem] shadow-sm border border-gray-100 overflow-hidden max-w-5xl">
        <div class="p-8 border-b border-gray-100 bg-gray-50/50">
            <h3 class="font-bold text-gray-900 text-lg flex items-center gap-2">
                <i class="fa-solid fa-image text-primary"></i> Detail Foto
            </h3>
        </div>
        <div class="p-8 lg:p-12">
            <form action="{{ route('admin.galleries.store') }}" method="POST" enctype="multipart/form-data" x-data="{ isSubmitting: false }" @submit="isSubmitting = true">
                @csrf
                ...
                <div class="mt-12 pt-8 border-t border-gray-100 flex items-center justify-between">
                    ...
                    <button type="submit" :disabled="isSubmitting" class="btn-primary py-3 px-8 shadow-lg shadow-primary/20 text-sm flex items-center gap-2 disabled:opacity-70">
                        ...
                        <i class="fa-solid fa-cloud-arrow-up" x-show="!isSubmitting"></i>
                        <span x-text="isSubmitting ? 'Mengunggah...' : 'Unggah Foto'"></span>
                    </button>
                </div>
            </form>
        </div>
